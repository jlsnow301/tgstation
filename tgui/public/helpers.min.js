(() => {
  ...
  Byond.callAsync = (path, params) => {
    ...
    Byond.call(
      path,
      assign({}, params, { callback: 'Byond.__callbacks__[' + index + ']' }),
    );
    return promise;
  };
  Byond.topic = (params) => Byond.call('', params);
  Byond.command = (command) => Byond.call('winset', { command: command });
  ...
  Byond.sendMessage = (type, payload) => {
    ...
    assign(message, { tgui: 1, window_id: Byond.windowId });
    Byond.topic(message);
  };
  ...
  Byond.saveBlob = (blob, filename, ext) => {
    if (window.navigator.msSaveBlob) {
      ...
      var opts = {
        ...
        types: [{ description: 'SS13 file', accept: accept }],
      };
      window
...
window.onerror = (msg, url, line, col, error) => {
  ...
  if (Byond.strictMode) {
    ...
    setInterval(setFatalErrorGeometry, 1e3);
  }
  if (Byond.strictMode) {
    Byond.sendMessage({ type: 'log', fatal: 1, message: stack });
  } else if (window.onerror.errorCount <= 1) {
    stack += '\nWindow is in non-strict mode, future errors are suppressed.';
    Byond.sendMessage({ type: 'log', message: stack });
  }
  if (Byond.strictMode) {
    window.update = () => {};
    window.update.queue = [];
...
window.replaceHtml = (inline_html) => {
  ...
  document.body.insertAdjacentHTML(
    'afterbegin',
    '\x3c!-- tgui:inline-html-start --\x3e' +
      inline_html +
      '\x3c!-- tgui:inline-html-end --\x3e',
  );
};
